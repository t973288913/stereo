...
def compute_mIoU(prediction, target,num_classes):  
    # 确保预测和目标的形状一致
    #prediction shape: (BS, H,W,CL,)
    # pdb.set_trace()
    ...
    for cls in range(num_classes):
        intersection = ((pred_classes == cls) & (target_classes == cls)).sum().item()
        union = ((pred_classes == cls) | (target_classes == cls)).sum().item()
        
        if union != 0:
            iou_per_class[cls] = intersection / union
        class_counts[cls] = (target_classes == cls).sum().item()
    
    # 计算每个类别的mIoU
    
    mIoU = iou_per_class[class_counts != 0].mean().item()
    return mIoU
