...
# def npy_disp_loader(path):
#     return readnpy(path)[0].astype(np.float32)
...
class BaseReader_mul(Dataset):
    def __init__(self, root, list_file, image_reader='PIL', disp_reader='PIL', seg_reader='seg', right_disp=True, occ_mask=False):
        self.root = root
        self.list_file = list_file
        self.image_reader_type = image_reader
        self.disp_reader_type = disp_reader
        self.seg_reader_type = seg_reader  #seg_reader_type
        ...
        self.seg_loader = self.build_seg_loader()
    ...
    def build_disp_loader(self):
        ...
        # elif self.disp_reader_type == 'npy':
        #     return npy_disp_loader

        else:
            raise NotImplementedError('Disp reader type not supported: {}'.format(self.disp_reader_type))

    def build_seg_loader(self):
            if self.seg_reader_type == 'seg':
                return seg_loader
            else:
                raise NotImplementedError('seg reader type not supported: {}'.format(self.seg_reader_type))
