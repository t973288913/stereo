...
def npy_disp_loader(path):
    ...
    cx = 320.0
    cy = 340.0
    left_ins = [fx, 0, cx, 0, fy, cy, 0, 0, 1]
    right_ins = [fx, 0, cx, 0, fy, cy, 0, 0, 1]
    
    left_gt_depth = np.load(path).astype(np.float32)
    left_gt_disp = baseline * left_ins[0] / (left_gt_depth + 0.000001)
