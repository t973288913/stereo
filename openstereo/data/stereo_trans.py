    ...
    def __call__(self, sample):
        ...
        if th > h or tw > w:
         # pad the arrays with zeros to the desired size
            ...
            for k in sample.keys():
                if k in ['left', 'right', 'disp', 'disp_right', 'occ_mask', 'occ_mask_right','labels']:
...
class NormalizeImage(object):
    ...
    def __call__(self, sample):
       ...
       # sample['disp'] = self.normalize_disparity(sample['disp'])
        return sample
    ...
    #def normalize_disparity(disp):
        print('Disparity Type:', type(disp))
        print('Disparity Shape:', disp.shape)
        # 处理视差数据的归一化步骤
        disp = disp / 192.0  # 假设最大视差为192
        return disp
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        for k in sample.keys():
            if k in ['left', 'right']:
                sample[k] = cv2.resize(sample[k], (w, h), interpolation=cv2.INTER_LINEAR)
            elif k in ['disp', 'disp_right', 'occ_mask', 'occ_mask_right']:
                sample[k] = cv2.resize(sample[k], (w, h), interpolation=cv2.INTER_NEAREST)
        return sample
