...
class StereoBatchDataset(Dataset):
    def __init__(self, data_cfg, scope='train'):
        ...
        self.dataset_disp = None  # 确保初始化 dataset_disp
        self.transform_disp = None  # 确保初始化 transform_disp
        # pdb.set_trace()
        self.build_dataset()
    ...
    def build_dataset(self):
        if self.data_cfg['name'] in ['KITTI2012', 'KITTI2015']:
        ...
        elif self.data_cfg['name'] in ['mul_only_disp']:
            print("enter mul_only_disp")
            if "val" == self.scope:
                
                from data.reader.kitti_reader import KittimulReader
                self.disp_reader_type = 'PIL'
                self.seg_reader_type  = 'seg'
                self.dataset = KittimulReader(
                    self.data_cfg['root'],
                    self.data_cfg[f'{self.scope}_list'],
                    self.image_reader_type,
                    self.disp_reader_type,
                    self.seg_reader_type,
                    right_disp=self.return_right_disp,
                    use_noc=self.data_cfg['use_noc'] if 'use_noc' in self.data_cfg else False,  # NOC disp or OCC disp
                )
           
            elif "train" == self.scope:
                # pdb.set_trace()
            ...
            elif "val_disp" == self.scope:
                # pdb.set_trace()
                from data.reader.kitti_reader import KittiReader
                # print("enter mul else")
                self.disp_reader_type = 'PIL'
                # self.disp_reader_type = 'npy'
                self.dataset = KittiReader(
                    self.data_cfg['root'],
        ...
        elif self.data_cfg['name'] in ['mul']:
            # print("enter mul")
            if "test" in self.scope:
                # print("enter mul test")
                from data.reader.kitti_reader import KittimulReader
                self.disp_reader_type = 'PIL'
        ...
        if 'val_disp' in self.scope:
            self.build_transform_disp()
            # pdb.set_trace()
            
        else:
            self.build_transform()
        # if 'transform_disp' in self.data_cfg:
        #     # pdb.set_trace()
    ...
    def build_transform_disp(self):
        transform_config = self.data_cfg['transform_disp']
        
        # Create a dictionary to map scope to the corresponding configuration
        # pdb.set_trace()
        scope_to_config = {
            'train_disp': transform_config['train_disp'],
            'val_disp': transform_config.get('val_disp', transform_config.get('test')),
            'test': transform_config.get('test'),
        }
        # Get the configuration based on the scope
        config = scope_to_config.get(f'{self.scope}')
        if config is None:
            # If the scope is not supported, raise a ValueError
            pdb.set_trace()
            raise NotImplementedError(f'{self.scope} is not supported yet.')
        self.transform_disp = self.build_transform_disp_by_cfg(config)
        # pdb.set_trace()
    ...
    def __getitem__(self, indexs):
        ...
        for index in indexs:
            if self.dataset:
                ...
                if self.transform:
                    ...
                    # pdb.set_trace()
                    result = self.transform_disp(sample)
        ...
        batch_result_disp['index'] = indexs
    ...
    def build_transform_by_cfg(self, transform_config):
        ...
        for trans in transform_config:
            if trans['type'] == 'CenterCrop':
                ...
                transform_compose.append(ST.Resize(trans['size']))
            ...
            # 打印当前的 transform_compose 列表内容
            print(f"Added transform: {trans['type']}, Current transform_compose: {transform_compose}")

        return ST.Compose(transform_compose)
        

    def build_transform_disp_by_cfg(self, transform_config):
        
        transform_compose_disp = []
 
        for trans in transform_config:
            if trans['type'] == 'CropOrPad':
                transform_compose_disp.append(ST.CropOrPad(trans['size']))
                if 'size-disp' in trans:
                    transform_compose_disp.append(ST.StereoPad(trans['size-disp']))
            elif trans['type'] == 'RandomCrop':
                # pdb.set_trace()
                transform_compose_disp.append(ST.RandomCrop(trans['size']))
                self.random_crop_index = len(transform_compose_disp) - 1
           
            elif trans['type'] == 'TransposeImage':
                transform_compose_disp.append(ST.TransposeImage())
            elif trans['type'] == 'ToTensor':
                transform_compose_disp.append(ST.ToTensor())
            elif trans['type'] == 'NormalizeImage':
                transform_compose_disp.append(ST.NormalizeImage(trans['mean'], trans['std']))
            elif trans['type'] == 'FlowAugmentor':
                transform_compose_disp.append(ST.FlowAugmentor(trans['size'], trans['min_scale'], trans['max_scale'], trans['do_flip']))
            elif trans['type'] == 'SparseFlowAugmentor':
                transform_compose_disp.append(ST.SparseFlowAugmentor(trans['size'], trans['min_scale'], trans['max_scale'], trans['do_flip']))
            elif trans['type'] == 'ColorTransform':
                transform_compose_disp.append(ST.ColorTransform(trans['range']))
            elif trans['type'] == 'EraserTransform':
                transform_compose_disp.append(ST.EraserTransform(trans['prob']))
            elif trans['type'] == 'SpatialTransform':
                transform_compose_disp.append(ST.SpatialTransform(trans['size'], trans['min_scale'], trans['max_scale'], trans['do_flip']))
            elif trans['type'] == 'RandomFlip':
                transform_compose_disp.append(ST.RandomFlip(trans['do_flip_type'], trans['h_flip_prob'], trans['v_flip_prob']))
            # 打印当前的 transform_compose 列表内容
            print(f"Added transform: {trans['type']}, Current transform_compose: {transform_compose_disp}")

        

        return ST.Compose(transform_compose_disp)
    ...
    def __len__(self):
        ...
        if self.dataset_disp:
            return len(self.dataset_disp)
        else:
            pdb.set_trace()
