...
def arg_parse():
    ...
    parser.add_argument('--master_port', type=str, default='12358', help="master port")
...
def worker(rank, world_size, opt, cfgs):
    ...
    if is_dist:
        ...
        find_unused_parameters = model_cfg.get('find_unused_parameters', True)
    ...
    # elif scope == 'eval-image':
    #     # Load the input images
    #     input_left_image = torch.load(opt.input_left_image).to(device)
    #     input_right_image = torch.load(opt.input_right_image).to(device)

    #     # Perform inference on the input images
    #     output_image = model_trainer.inference(input_left_image, input_right_image)
